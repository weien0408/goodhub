...
MobileAimBtn.Name = "MobileAimButton"
...
Title.Text = "GOOD HUB (VISIBLE MOD)"
...
local function CreateSlider(text, max, settingKey)
    ...
    RunService.RenderStepped:Connect(function()
        if dragging then
            ...
            local ratio = math.clamp((mouseLoc.X - Bar.AbsolutePosition.X) / Bar.AbsoluteSize.X, 0, 1)
...
UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingUI = false end end)
...
-- [[ 修改後的 GetClosestTarget：加入 IsVisible 檢查 ]] --
